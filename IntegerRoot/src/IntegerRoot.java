...
public final class IntegerRoot {
    ...
    private static int root(int n, int r) {
        ...
        int low = 0; // Initialize the lower bound of the search interval
        int high = n + 1; // Initialize the upper bound of the search interval
        int mid = (low + high) / 2;

        // Use interval halving to find the root
        while (high - low > 1) {

            // Check if mid^r is greater than or equal to n
            if (power(mid, r) > n) {
                high = mid; // Update the upper bound to the middle
            } else {
                low = mid; // Update the lower bound to the middle + 1
            }
