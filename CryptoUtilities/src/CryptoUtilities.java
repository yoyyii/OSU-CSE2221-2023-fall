...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //divide m until the remainder is zero
        if (!m.isZero()) {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        //if n is divisible by two, n is even (return true). Return false otherwise
        boolean result = true;
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber copyP = new NaturalNumber2();
        copyP.copyFrom(p);

        NaturalNumber oldN = new NaturalNumber2();
        oldN.copyFrom(n);

        if (copyP.isZero()) {
            n.copyFrom(one);
        } else if (copyP.compareTo(one) == 0) {
            ...
            if (!isEven(p)) {//if p is odd, we have to multiply an additional time
                n.multiply(oldN);
            }
            n.copyFrom(n.divide(m));
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber copyW = new NaturalNumber2();
        copyW.copyFrom(w);
        NaturalNumber copyW1 = new NaturalNumber2();
        copyW1.copyFrom(w);

        NaturalNumber copyN = new NaturalNumber2();
        copyN.copyFrom(n);
        NaturalNumber copyN1 = new NaturalNumber2();
        copyN1.copyFrom(n);

        final NaturalNumber two = new NaturalNumber2(2);
        final NaturalNumber one = new NaturalNumber2(1);

        //check if conditions of the ensures clause is true (if w is a witness)

        powerMod(copyW, two, copyN);
        copyN1.subtract(one);
        powerMod(copyW1, copyN1, copyN);

        if (copyW.compareTo(one) == 0 || copyW1.compareTo(one) != 0) {
            result = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            //generate different witness test cases. If all test cases witness n, n is not a prime number
            for (int i = 0; i < 50 && isPrime == true; i++) {
                NaturalNumber witness = randomNumber(copyN);

                while (witness.compareTo(one) <= 0) {

                    witness = randomNumber(copyN);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        if (isEven(n) && n.compareTo(two) != 0) {
            n.increment();
        } //test if n is a non-two even number. If it is, increment n to next odd number

        //loop until we find the next prime number
        while (!isPrime2(n)) {

            n.add(two); // if n is odd, add two to get to next odd number
        }
    }
