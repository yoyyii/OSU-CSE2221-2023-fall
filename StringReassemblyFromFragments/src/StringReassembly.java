...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //combination = str1[0, |str1|-overlap) * str2
        String str1Comb = str1.substring(0, str1.length() - overlap);

        String combination = str1Comb + str2;
     ...
     *
     * everything in the set can not have overlap with each other
     *
     * @param strSet
     ...
     * @updates strSet
     * @requires CONTAINS_NO_SUBSTRING_PAIRS(strSet) CONTAINS_NO_SUBSTRING_PAIRS
     *           (strSet: finite set of string of character): boolean is for all
     *           t: string of character where (t is in strSet)
     *           (SUBSTRINGS(strSet \ {t}, t) = {})
     * @ensures <pre>
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //check str is not a a substring of any string in set
        for (String t : strSet) {
            if (t.contains(str)) {
                exist = true;
            }
        ...
        //if str is not a substring of any strings in strSet, add str into set
        if (exist == false) {
            strSet.add(str);
            Set<String> strSetCopy = strSet.newInstance();
            strSetCopy.transferFrom(strSet);

            //check that no string in strSet is a substring of str. If it is, remove it from the set
            for (String t : strSetCopy) {
                if (!str.contains(t) || str.equals(t)) {
                    strSet.add(t);
                }
            }
        }
     ...
     * @updates strSet
     * @requires CONTAINS_NO_SUBSTRING_PAIRS(strSet) CONTAINS_NO_SUBSTRING_PAIRS
     *           ( strSet: finite set of string of character ) : boolean is for
     *           all t: string of character where (t is in strSet)
     *           (SUBSTRINGS(strSet \ {t}, t) = {})
     * @ensures <pre>
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        String textReplace = textCopy.replace("~", "\n");

        out.println(textReplace);
